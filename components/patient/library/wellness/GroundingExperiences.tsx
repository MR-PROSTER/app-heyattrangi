...
const SENSES = [
  { count: 5, sense: "see", prompt: "Name 5 things you can see", placeholder: "I see…" },
  { count: 4, sense: "feel", prompt: "Name 4 things you can feel/touch", placeholder: "I feel…" },
  { count: 3, sense: "hear", prompt: "Name 3 things you can hear", placeholder: "I hear…" },
  { count: 2, sense: "smell", prompt: "Name 2 things you can smell", placeholder: "I smell…" },
  { count: 1, sense: "taste", prompt: "Name 1 thing you can taste", placeholder: "I taste…" },
]

export function Grounding54321Experience(props: ExperiencePlayerProps) {
  const [step, setStep] = useState(0)
  const [checked, setChecked] = useState<Record<string, boolean>>({})
  const [completed, setCompleted] = useState(false)
  const current = SENSES[step]
  const keys = Array.from({ length: current.count }, (_, i) => `${step}-${i}`)
  const allChecked = keys.every((k) => checked[k])
  const progress = completed ? 100 : ((step + (allChecked ? 1 : 0.4)) / SENSES.length) * 100
  ...
  return (
    <ExperienceShell
      ...
      color={props.color}
      progress={progress}
      progressLabel={`${step + 1}/${SENSES.length}`}
      onExit={props.onExit}
      footer={
        <ExperienceControls
          ...
          nextLabel={step >= SENSES.length - 1 ? "Finish" : "Next"}
          showRestart
          onRestart={restart}
        />
      }
    >
      <div className="text-white py-2">
        <p className="text-center text-white/60 text-[11px] font-black uppercase tracking-[0.2em] mb-2">
          {current.count}-{SENSES.length - step > 1 ? "…" : ""} sense check
        </p>
        <h3 className="text-center text-2xl font-bold mb-6">{current.prompt}</h3>
        <ul className="space-y-3">
          {keys.map((key, i) => (
            <li key={key}>
              <label className={`flex items-center gap-3 rounded-2xl border px-4 py-3.5 cursor-pointer transition-all ${
                checked[key] ? "bg-white/20 border-white/40" : "bg-white/10 border-white/15 hover:bg-white/15"
              }`}>
                <input
                  type="checkbox"
        ...
        {!allChecked && (
          <p className="text-center text-white/50 text-xs mt-4">Check each item before continuing</p>
        )}
      </div>
    </ExperienceShell>
  )
