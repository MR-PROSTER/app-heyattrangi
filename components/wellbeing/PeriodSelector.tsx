...
export default function PeriodSelector({
  ...
  const label = type === "week" ? "Week" : "Month"
    ...
    <div ref={dropdownRef} className="relative z-30">
      ...
      >
        ...
        <button
          ...
          type="button"
          aria-label="View monthly wellbeing"
          className="w-8 h-8 rounded-full bg-[#2A1810] hover:bg-[#3d2417] active:scale-95 flex items-center justify-center text-white shrink-0 shadow-sm transition-all duration-200 hover:scale-105"
        >
          ...
          {items.map((item) => (
            <li key={item}>
              <button
                ...
                `}
              >
                {type === "week" ? `Week ${item}` : MONTH_NAMES[item]}
              </button>
            </li>
          ))}
        </ul>
