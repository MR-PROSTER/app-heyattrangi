...
export default function SignUpPage() {
  ...
  const handleEmailContinue = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Verify that user doesn't already exist for SignUp flow
      ...
      // 2. If user doesn't exist, send OTP verification code
  ...
  const handleCodeVerify = async (e: React.FormEvent) => {
    ...
    try {
      // For sign up, register the user as PATIENT role
  ...
  const handleGoogleSignIn = async () => {
    ...
    setError("")
    try {
      // During signup, route through callback specifying patient signup role
      const callbackUrl = "/auth/callback?signup=true&role=PATIENT"
      await signIn("google", {
        callbackUrl,
        redirect: true,
      })
        ...
        <div className="w-full max-w-[420px]">
          ...
          {signedIn ? (
            <div className="mb-8 w-full flex flex-col items-center justify-center py-10">
              ...
              <p className="text-sm text-gray-500 font-sans">Getting your onboarding ready</p>
            </div>
          ) : (
